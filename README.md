...
영수증 기반 소비패턴 분석 웹 서비스 (5인 팀 프로젝트)
...
- 회원가입/로그인 도메인 MyBatis → JPA 마이그레이션
- Redis Stream 기반 비동기 파이프라인 설계 및 트러블슈팅
...
- Redis Stream 기반 비동기 파이프라인 도입으로 응답 시간을 약 20~30초에서 4~5초 수준으로 단축 (약 80% 개선)
...
├── docker-compose.yml        # 컨테이너 구성 (Podman 호환)
...
**문제**: Redis Stream 기반 비동기 처리 과정에서 설정 파일 누락으로 인해 간헐적으로 처리 실패가 발생했습니다.

**원인**: 비동기 파이프라인 구성에 필요한 설정이 누락되어, 특정 상황에서 메시지 전달 및 처리 흐름이 중단되었습니다. 간헐적으로 실패하는 증상이었기 때문에 단순 오류보다 원인 파악이 훨씬 어려웠습니다.

**해결**: 원인을 3~6일에 걸쳐 추적한 결과, 누락된 설정 파일과 연결 구성을 확인하고 정상 동작하도록 보완했습니다. 이후 비동기 처리 흐름이 안정적으로 동작하며 응답시간을 20~30초에서 4~5초로 단축했습니다.

### 2. MyBatis → JPA 마이그레이션 시 save()가 INSERT 대신 UPDATE로 동작한 문제

**문제**: MyBatis 기반 구조에서 JPA로 마이그레이션한 뒤, 엔티티 저장 시 `save()`가 기대와 다르게 UPDATE로 동작했습니다.

**원인**: 새 엔티티인지 여부를 판단하는 로직이 명확하지 않아, JPA가 기존 엔티티로 오인하는 문제가 있었습니다.

**해결**: `Persistable` 인터페이스를 구현하고 `isNew()` 메서드를 명시적으로 정의하여, 신규 엔티티는 INSERT, 기존 엔티티는 UPDATE로 분기되도록 수정했습니다.
